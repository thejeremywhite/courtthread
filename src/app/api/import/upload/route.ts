...
async function importConversation(
  ...
  // messages are still present (e.g. someone who left the group). Previously those senders
  // got a throwaway id generated below with no matching participants row — every message
  // from them then showed a blank sender name everywhere (Media grid, conversation view,
  // exports). Backfill a real participant row for anyone found only via their messages.
  for (const msg of conv.messages) {
